...
function songPitchBounds(song: SongBundle, keyShift: number) {
  ...
  const displayMinMidi = songMinMidi + keyShift - 4;
  const displayMaxMidi = songMaxMidi + keyShift + 4;
  const minHz = midiToHz(displayMinMidi);
  const maxHz = midiToHz(displayMaxMidi);
  return {
    displayMinMidi,
    displayMaxMidi,
    minHz,
...
function foldToNearestOctave(liveHz: number, targetHz: number): number {
  let bestDiff = Infinity;
  let bestPitch = liveHz;
  for (let o = -3; o <= 3; o++) {
    const candidate = liveHz * Math.pow(2, o);
    const diff = Math.abs(Math.log2(candidate / targetHz));
    if (diff < bestDiff) {
      bestDiff = diff;
      bestPitch = candidate;
    }
...
export default function PitchLane({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const bounds = songPitchBounds(currentSong, shift);
      ...
      if (pitchHz > 0) {
        let trailHz = pitchHz;
        if (active) {
          trailHz = foldToNearestOctave(
            pitchHz,
            shiftHz(active.expectedHz, shift)
          );
        }
        const last = lastTrailSampleRef.current;
        if (
          Math.abs(elapsedSec - last.t) > 0.02 ||
          Math.abs(trailHz - last.hz) > 0.5
        ) {
          trailRef.current.push({ t: elapsedSec, hz: trailHz });
          lastTrailSampleRef.current = { t: elapsedSec, hz: trailHz };
      ...
      const yForHz = (hz: number): number => {
        const clamped = Math.max(bounds.minHz, Math.min(bounds.maxHz, hz));
        return (
          PAD_Y +
          (1 - (Math.log2(clamped) - bounds.logMin) / (bounds.logMax - bounds.logMin)) *
            (H - PAD_Y * 2)
        );
      };
      ...
      for (let midi = bounds.displayMinMidi; midi <= bounds.displayMaxMidi; midi++) {
      ...
      for (const syl of currentSong.syllables) {
        ...
        if (boxW > 14) {
          ...
          const fontSize = Math.max(
            9,
            Math.min(13, boxW / Math.max(word.length, 1) * 1.1)
          );
      ...
      if (pitchHz > 0) {
        ...
        let displayHz = pitchHz;
        if (active) {
          displayHz = foldToNearestOctave(
            pitchHz,
            shiftHz(active.expectedHz, shift)
          );
        }
        const dotY = roundPx(yForHz(displayHz));
        const R = 10;
